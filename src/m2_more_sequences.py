...
def shortest_string(strings):
    ...
    minimum = [len(strings[0]),0] #minimum is list that holds length of string and its index
    for k in range(len(strings)):
        if len(strings[k]) < minimum[0]:
            minimum[0] = len(strings[k])
            minimum[1] = k

    return strings[minimum[1]]
    # ------------------------------------------------------------------
...
def index_of_largest_number(numbers, n):
    ...
    """
    maximum = [numbers[0], 0]  # minimum is list that holds length of string and its index
    for k in range(n):
        if numbers[k] > maximum[0]:
            maximum[0] = numbers[k]
            maximum[1] = k
...
def is_palindrome(s):
    ...
    Type hints:
      :type s: str
    """
    #Creates reverse word to check against
    pal = ''
